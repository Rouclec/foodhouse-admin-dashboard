...
type Config struct {
	...
	Sms struct {
		Twilio struct {
			AccountSID string `conf:"env:TWILIO_ACCOUNT_SID,required"`

			AuthToken string `conf:"env:TWILIO_AUTH_TOKEN,required"`
...
func run(ctx context.Context, logger zerolog.Logger) error {
	...
	if config.Sms.GoogleChat.WebhookURL != "" {
		...
		if smsErr != nil {
			return fmt.Errorf("failed to create google chat webhook sender: %w", smsErr)
		}
		smsSender.AddSmsSender(googleChatWebhookSender)
	}

	sess, err := session.NewSessionWithOptions(session.Options{
		SharedConfigState: session.SharedConfigEnable,
		Config:            aws.Config{Region: &config.AwsRegion},
	})
	if err != nil {
	...
	if config.Sms.SnsTopicArn != "" {
		snsSending := sms.NewSmsSenderSnsDebug(config.Sms.SnsTopicArn, snsSvc)
		smsSender.AddSmsSender(snsSending)
	}
